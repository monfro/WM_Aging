...
cd([sourcePath]);
TE = fb_RB_ME_PAID_Dicom2Nifti(pwd, numberOfRuns, size(TE,2));
filesToBeMoved = dir('*.nii');
filesToBeMoved = char(filesToBeMoved.name);
fb_fileMoveForLinux(sourcePath, [targetPath '/converted_Volumes'], filesToBeMoved, 100);
...
if isempty(prescanPath) %% there is no prescan
    ...
    filesToBeMoved = char(filesToBeMoved.name);
    fb_fileMoveForLinux([targetPath '/converted_Weight_Volumes'], [targetPath '/converted_Volumes'], filesToBeMoved, 1000);
    ...
    fb_fileMoveForLinux([targetPath '/converted_Volumes/'], [targetPath '/converted_Weight_Volumes'], filesPrescan(:,1:end-2,1), 1000);
...
%     spm_reslice(resliceFiles);
    ...
    for i=1:size(TE,2)        
        fb_fileMoveForLinux([targetPath '/converted_Weight_Volumes'], [targetPath '/converted_Volumes'], filesPrescan(:,1:end-2,i), 1000);
    end
    ...
    for i=1:size(TE,2)
        fb_fileMoveForLinux([targetPath '/converted_Volumes'], [targetPath '/converted_Weight_Volumes'], filesPrescan(:,1:end-2,i), 1000);
        fb_fileMoveForLinux([targetPath '/converted_Volumes'], [targetPath '/converted_Weight_Volumes'], cat(2,repmat('r',[WeightVolumes 1]),filesPrescan(:,1:end-2,i)), 1000);
    end
...
filesToBeMoved = char(filesToBeMoved.name);
fb_fileMoveForLinux([targetPath '/converted_Volumes'], [targetPath '/PAID_data'], filesToBeMoved, 1000);
...
function status=fb_fileMoveForLinux(sourceDirectory,targetDirectory,filesToBeMoved,filePerCycle)

cd(sourceDirectory);
% make the source and target directories space-friendly (otherwise, linux
% crashes when there are spaces in the directory)
sourceDirectoryForLinux = strrep(sourceDirectory,' ','\ ');
targetDirectoryForLinux = strrep(targetDirectory,' ','\ ');

filesToBeMoved = cat(2, repmat(sourceDirectoryForLinux,size(filesToBeMoved,1),1), repmat('/',size(filesToBeMoved,1),1), filesToBeMoved,...
...
for i=1:ceil(size(filesToBeMoved,1)/filePerCycle)
    if i==ceil(size(filesToBeMoved,1)/filePerCycle)
        unix(['mv ' reshape(permute(filesToBeMoved((i-1)*filePerCycle+1:end,:),[2 1]), 1 , size(filesToBeMoved,2)*(size(filesToBeMoved,1)-(i-1)*filePerCycle))...
                targetDirectoryForLinux]);
    else
        unix(['mv ' reshape(permute(filesToBeMoved((i-1)*filePerCycle+1:i*filePerCycle,:),[2 1]), 1, size(filesToBeMoved,2)*filePerCycle) targetDirectoryForLinux]);
    end
end
